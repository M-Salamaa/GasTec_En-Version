...
namespace WebApp_gastec.Controllers
{
    public class LubricantsController : Controller
    {
        ...
        public LubricantsController(IWebHostEnvironment hostingEnviroment)
        {
        ...
        public async Task<IActionResult> Index(string ID_)
        {
            ...
            {
                await CachedAllImagesAsync(model, "Lubricants-ar");
                CachedAllHtmlLinks(model, "Lubricants-ar");
            }
            else
            {
                await CachedAllImagesAsync(model, "Lubricants-en");
                CachedAllHtmlLinks(model, "Lubricants-en");
            }
            return View(model);
        ...
        public async Task<IActionResult> SubCommercialAsync(string OilID_)
        {
            ...
            if (Gastech_Vault.TranslationLanguageID == 0)
            {
                await CachedAllImagesAsync(model, "Commerical_ar");
                CachedAllHtmlLinks(model, "Commerical-ar");
            }
            else
            {
                await CachedAllImagesAsync(model, "Commerical_en");
                CachedAllHtmlLinks(model, "Commerical_en");
            }
            return View(model);
        }
        ...
        public async Task<IActionResult> SubIndustrialAsync(string OilID_)
        {
            ...
            if (Gastech_Vault.TranslationLanguageID == 0)
            {
                await CachedAllImagesAsync(model, "Industrial_ar");
                CachedAllHtmlLinks(model, "Industrial_ar");
            }
            else
            {
                await CachedAllImagesAsync(model, "Industrial_en");
                CachedAllHtmlLinks(model, "Industrial_en");
            }
            return View(model);
        }
